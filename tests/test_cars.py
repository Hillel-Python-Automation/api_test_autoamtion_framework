...
class TestCars:
    def test_create_new_car(self, sign_up_response, headers):
        payload = json.dumps({
            "carBrandId": 1,
            "carModelId": 1,
            "mileage": 1
    ...
    def test_get_car_brand(self, sign_up_response, headers):
        response = car_api.get_car_brand(headers=headers)

        assert response.is_status_code(200)
        assert response.get_field('status') == 'ok'

    def test_get_car_brand_by_id(self, sign_up_response, headers):
    ...
    def test_get_cars_model(self, sign_up_response, headers):
        response = car_api.get_cars_model(headers=headers)

        assert response.is_status_code(200)
        assert response.get_field('status') == 'ok'

    def test_get_car_model_by_id(self, sign_up_response, headers):
    ...
    def test_get_current_user_car_by_id(self, sign_up_response, headers, car_id):
        response = car_api.get_current_user_car_by_id(headers=headers, created_car_id=car_id)

        assert response.is_status_code(200)
        assert response.get_field('status') == 'ok'

    def test_edit_existing_car(self, sign_up_response, headers, car_id):
        payload = json.dumps({
            "carBrandId": 2,
            "carModelId": 6,
            "mileage": 168223
    ...
    def test_delete_existing_car(self, sign_up_response, headers, car_id):
